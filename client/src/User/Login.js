...
const Login=()=> {
  ...
  const sendEmail = (e) => {
    emailjs.sendForm('service_j5zvqhi', 'template_3ummppf', e.target, 'bqB6goWLStLHMkilH')
      .then((result) => {
  ...
  const sendRequest = async (type = 'login') => {
    const res = await axios.post(`http://localhost:8000/user/${type}`, {
      name: inputs.name,
      email: inputs.email,
      ...
      <section class="overflow-hidden" >
        <div class="container px-4 py-5 px-md-5 text-center text-lg-start my-2">
          <div class="row gx-lg-5 align-items-center mb-3">
            <div class="col-6 mb-5 mb-lg-0" style={{ margin: "2.5rem 0rem 0rem 15rem" }}>
              <div class="position-absolute rounded-circle shadow-5-strong"></div>
              <div class="position-absolute shadow-5-strong"></div>
              <div class="card bg-glass">
                <div class="card-body px-4 py-4 md-6 ">
                  <form onSubmit={handleSubmit}>
                    <div class="row">
                      <h4 style={{ textAlign: "center" }}>{isSignup ? "Signup" : "Login"}</h4>
                    ...
                    {isSignup ? (<button className="btn btn-primary" onClick={()=>{alert('Registration Successful')}} type="submit">Register</button>) : 
                    ...
                    {!isSignup && (<p>Are you an Admin User? Login <Link to='/adminlogin'>Here</Link></p>)}{" "}
                  </form>
                </div>
              </div>
            </div>
